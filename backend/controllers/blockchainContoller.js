...
const upgradeLotteryTicketCount = async (type) => {
  ...
  try {
    ...
    lotteryDataFromBlockchain = JSON.parse(
      JSON.stringify(lotteryData, (key, value) =>
  ...
  try {
    lotteryDataFromDB = await lottery
      .findOne({ lotteryId: lotteryDataFromBlockchain[0] })
      .populate("purchases", "tax");
    ...
    for (let i = ticektSoldBlockchain.length - 1; i >= 0; i--) {
      ...
      if (!lotteryNumbers.includes(ticketString)) {
        ...
        for (let i = 0; i < ticketString.length; i += 2) {
          ...
          ticketNumbersArray.push(
            isValidNumber ? parseInt(twoDigitString, 10) : 0
          );
        }
      ...
      if (difference === 0) {
        ...
        return {
          msg: "not match -- done",
          bc: ticektSoldBlockchain.length,
          db: ticektSoldB,
          bc2: ticektSoldBlockchain,
          db2: lotteryNumbers,
          purchaseDataArray: purchaseDataArray,
        };
      }
    }
  } else {
