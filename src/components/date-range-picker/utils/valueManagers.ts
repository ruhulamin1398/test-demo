...
export const rangeValueManager: RangePickerValueManager = {
  ...
  cleanValue: (utils, value) =>
    value.map((date) =>
      replaceInvalidDateByNull(utils, date)
    ) as PickerRangeValue,
  ...
  getTimezone: (utils, value) => {
    const timezoneStart = utils.isValid(value[0])
      ? utils.getTimezone(value[0])
      : null;
    const timezoneEnd = utils.isValid(value[1])
      ? utils.getTimezone(value[1])
      : null;
...
export const getRangeFieldValueManager = ({
  ...
  getDateFromSection: (value, activeSection) =>
    value[getActiveDateIndex(activeSection)],
  ...
  clearDateSections: (sections, activeSection) => {
    ...
    if (getActiveDateIndex(activeSection) === 0) {
      return [
        ...dateRangeSections.startDate.map((section) => ({
          ...section,
          value: "",
        })),
        ...dateRangeSections.endDate,
    ...
    return [
      ...dateRangeSections.startDate,
      ...dateRangeSections.endDate.map((section) => ({
        ...section,
        value: "",
      })),
    ];
