...
function useDateRangeCalendarDefaultizedProps(
  ...
  const validationProps =
    useApplyDefaultValuesToDateValidationProps(themeProps);
...
const DateRangeCalendar = React.forwardRef(function DateRangeCalendar(
  ...
  const { view, setFocusedView, focusedView, setValueAndGoToNextView } =
    useViews({
      view: inView,
      views,
      openTo,
  ...
  const { rangePosition, setRangePosition } = useRangePosition({
    ...
    onRangePositionChange:
      onRangePositionChangeProp ?? rangePositionContext?.setRangePosition,
  });
  ...
  const { calendarState, setVisibleDate, onMonthSwitchingAnimationEnd } =
    useCalendarState({
      ...
      getCurrentMonthFromVisibleDate: (visibleDate, prevMonth) => {
        const firstVisibleMonth = utils.addMonths(
          prevMonth,
          1 - currentMonthCalendarPosition
        );
        const lastVisibleMonth = utils.endOfMonth(
          utils.addMonths(firstVisibleMonth, calendars - 1)
        );

        // The new focused day is inside the visible calendars,
        // Do not change the current month
        if (
          utils.isWithinRange(visibleDate, [
            firstVisibleMonth,
            lastVisibleMonth,
          ])
        ) {
        ...
        if (utils.isAfter(visibleDate, lastVisibleMonth)) {
          return utils.startOfMonth(
            utils.addMonths(visibleDate, currentMonthCalendarPosition - 1)
          );
        }
  ...
  > = useSlotProps({
    ...
    additionalProps: {
      ...
      onMonthChange: (month) =>
        setVisibleDate({ target: month, reason: "header-navigation" }),
      minDate,
      maxDate,
      disabled,
      disablePast,
  ...
  const [rangePreviewDay, setRangePreviewDay] =
    React.useState<PickerValidDate | null>(null);
  ...
  const handleDayMouseEnter = useEventCallback(
    (
      ...
      newRangePreviewDay: PickerValidDate
    ) => {
